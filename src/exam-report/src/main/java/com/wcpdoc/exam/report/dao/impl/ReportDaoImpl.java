...
public class ReportDaoImpl extends RBaseDaoImpl<Object> implements ReportDao {
	...
	public List<Map<String, Object>> examStatis(Integer examId) {
		String sql = "SELECT EXAM.`NAME` EXAM_NAME, EXAM.START_TIME EXAM_START_TIME, EXAM.END_TIME EXAM_END_TIME, EXAM.MARK_START_TIME EXAM_MARK_START_TIME, "
				+ "EXAM.MARK_END_TIME EXAM_MARK_END_TIME, COUNT( MY_EXAM.ID ) AS EXAM_USER_NUM, COUNT( MY_EXAM.STATE = 1 ) AS MISS_USER_NUM, "
	...
	public List<Map<String, Object>> myExamListpage(Integer examId) {
		String sql = "SELECT MY_EXAM.`NO` AS MY_EXAM_NO, USER.ID AS USER_ID, USER.`NAME` AS USER_NAME, USER.ORG_ID AS ORG_ID, "
				...
				+ "(SELECT MARK_USER.`NAME` FROM SYS_USER AS MARK_USER WHERE MARK_USER.ID = MY_EXAM.MARK_USER_ID ) AS MY_MARK_USER_NAME "
	...
	public List<Map<String, Object>> questionListpage(Integer examId) {
		String sql = "SELECT QUESTION.ID QUESTION_ID, QUESTION.TITLE QUESTION_TITLE, COUNT(MY_EXAM_DETAIL.USER_ID) USER_NUM, "
				+ "SUM(MY_EXAM_DETAIL.SCORE = MY_EXAM_DETAIL.QUESTION_SCORE) SUCC_USER_NUM FROM EXM_MY_EXAM_DETAIL MY_EXAM_DETAIL "
