...
//				+ "IFNULL(SUM(CASE WHEN (MY_EXAM.STATE = 1 AND MY_EXAM.MARK_STATE != 3) OR MY_EXAM.STATE = 2 THEN 1 ELSE 0 END), 0) AS UN_EXAM_NUM, "// 待考考试数量
	...
	@Override
	public Map<String, Object> subAdminHome(Integer userId) {
		String sql = "SELECT "
				+ "(SELECT COUNT(*) FROM EXM_EXAM WHERE (STATE = 1 OR STATE = 2) AND CREATE_USER_ID = :CUID) AS EXAM_NUM, "// 考试数量
				+ "(SELECT COUNT(*) FROM EXM_QUESTION WHERE STATE = 1 AND CREATE_USER_ID = :CUID2) AS QUESTION_NUM, "// 试题数量
				+ "(SELECT COUNT(*) FROM EXM_EXER WHERE STATE = 1 AND CREATE_USER_ID = :CUID3) AS EXER_NUM, "// 练习数量
				+ "(SELECT LENGTH(USER_IDS) - LENGTH(REPLACE(USER_IDS,',','')) - 1 FROM SYS_USER WHERE ID = :CRID4) AS USER_NUM "// 用户数量
				+ "FROM DUAL";
		return getMapList(sql, new Object[]{ userId, userId, userId, userId }).get(0);
	}
