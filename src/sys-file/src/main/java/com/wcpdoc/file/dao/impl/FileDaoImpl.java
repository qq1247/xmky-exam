...
public class FileDaoImpl extends RBaseDaoImpl<File> implements FileDao {
	...
	public PageOut getListpage(PageIn pageIn) {
		...
		sqlUtil.addWhere(ValidateUtil.isValid(pageIn.get("name")), "FILE.NAME LIKE :NAME", "%" + pageIn.get("name") + "%");
		sqlUtil.addWhere(ValidateUtil.isValid(pageIn.get("extName")), "FILE.EXT_NAME LIKE :EXT_NAME", "%" + pageIn.get("extName") + "%");
