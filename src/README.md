# 后端源码
## v3.8.0 cy 2022-04-29 10:07:00
###### 新增功能：支持人员机构导入导出
###### 优化改进：交卷后直接出成绩，如果有人工阅卷，显示之职能部分成绩
###### 优化改进：允许提前延后考试时间或阅卷时间
###### 优化改进：支持冻结用户（冻结后强制下线）
###### 优化改进：添加定时任务，已删除的试题，如果未被引用，则物理删除，级联图片
###### 优化改进：getListpage(PageIn pageIn)改造，接收自定义参数，解耦并减少sql重复
###### bug修复：dao层替换成命名参数的形式
###### bug修复：考试设置为不显示成绩，首页不应该统计相关信息
###### bug修复：win转linux迁移，程序启动不了问题
###### bug修复：复制试题的时候，如果里面有图片则复制一个图片。
###### bug修复：如果是中文环境，程序启动数据库报错信息展示中文
###### bug修复：处理sql注入风险（改造工具类）
###### bug修复：查询总数的sql去掉所有列，改为count(*)
###### bug修复：试卷设置单题分数实现错误
