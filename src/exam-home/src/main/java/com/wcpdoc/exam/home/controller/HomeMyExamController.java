...
public class HomeMyExamController extends BaseController{
	...
	@Resource
	private HomeMyExamService homeMyExamService;
	...
	public PageOut list(PageIn pageIn) {
		try {
			pageIn.setTen(getCurrentUser().getId() + "");
			return homeMyExamService.getMyExamListpage(pageIn);
		} catch (Exception e) {
	...
	public String toPaper(Model model, Integer examUserId) {
		try {
			homeMyExamService.toPaper(model, getCurrentUser(), examUserId);
	...
	public PageResult updateAnswer(Integer examUserQuestionId, String answer) {
		try {
			homeMyExamService.updateAnswer(getCurrentUser(), examUserQuestionId, answer);
	...
	public PageResult doPaper(Integer examUserId) {
		try {
			homeMyExamService.doPaper(getCurrentUser(), examUserId);
