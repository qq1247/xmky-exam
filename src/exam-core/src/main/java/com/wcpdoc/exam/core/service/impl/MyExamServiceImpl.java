...
public class MyExamServiceImpl extends BaseServiceImp<MyExam> implements MyExamService {
	...
	public void finish(Integer examId, Integer userId) {
		...
		for (MyQuestion myQuestion : myQuestionList) {
			...
			myQuestionService.updateById(myQuestion);// 更新每道题的分数（没作答也都标记为0分。影响的地方为人工阅卷时，所有题都有分数，才允许阅卷完成。）
	...
	public void doExam(Integer examId) {
		...
		Exam exam = examService.getById(examId);
		doExamTimedelay(exam);// 业务上变更考试结束时间为当前时间时，自动阅卷概率性校验为考试未结束（数据库时间四舍五入后比当前时间大。例：传入值：2022-05-12
								// 23:59:59,999999，保存为 2022-05-13 00:00:00），所以先延时后校验

		// 校验数据有效性
		doExamValid(exam);

		// 获取考试用户列表
		List<MyExam> myExamList = getList(examId);
	...
	public void doMark(Integer examId) {
		...
		// 获取考试用户列表
		List<MyExam> myExamList = getList(examId);// 考试用户列表
		...
		for (MyExam myExam : myExamList) {
			...
			BigDecimal passScore = BigDecimalUtil.newInstance(exam.getTotalScore()).mul(exam.getPassScore()).div(100, 2)
					.getResult();
			myExam.setAnswerState(
					BigDecimalUtil.newInstance(totalScore.getResult()).sub(passScore).getResult().doubleValue() >= 0 ? 1
							: 2);// 标记及格状态
			myExam.setMarkState(3);// 标记为阅卷结束
...
//		});
		...
		for (int i = 0; i < myExamWithSortList.size(); i++) {
			...
			updateById(myExamWithSortList.get(i));
		}
	}
...
//		});
	 ...
	 * @param myQuestion         我的试题 void
	 */
	private void multipleChoiceHandle(Question question, List<QuestionAnswer> questionAnswerList,
	...
	public List<Exam> getExamList(Integer userId) {
		List<Integer> examIdList = myExamDao.getList2(userId).stream().map(m -> m.getId()).collect(Collectors.toList());
		return examService.listByIds(examIdList).stream().filter(p -> p.getState() != 0).collect(Collectors.toList());
	}
