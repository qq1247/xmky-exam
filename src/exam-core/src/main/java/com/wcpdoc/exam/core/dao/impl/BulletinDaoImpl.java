...
public class BulletinDaoImpl extends RBaseDaoImpl<Bulletin> implements BulletinDao {
	...
	public PageOut getListpage(PageIn pageIn) {
		...
		sqlUtil.addWhere(ValidateUtil.isValid(pageIn.get("title")), "BULLETIN.TITLE LIKE :BULLETIN.TITLE", "%" + pageIn.get("title") + "%")
		.addWhere(ValidateUtil.isValid(pageIn.get("showType")), "BULLETIN.SHOW_TYPE = :BULLETIN.SHOW_TYPE", pageIn.get("showType", Integer.class))
			   .addWhere(pageIn.get("curUserId", Integer.class) == 1, "BULLETIN.UPDATE_USER_ID = :BULLETIN.UPDATE_USER_ID", pageIn.get("curUserId", Integer.class))// 如果是管理员，查询所有的
			   .addWhere(pageIn.get("curUserId", Integer.class) != 1,
				   	"(BULLETIN.READ_USER_IDS IS NULL OR BULLETIN.READ_USER_IDS LIKE :BULLETIN.READ_USER_IDS) AND BULLETIN.START_TIME <= :BULLETIN.START_TIME AND :BULLETIN.END_TIME <= BULLETIN.END_TIME AND BULLETIN.STATE = 1", 
			   ...
			   .addWhere(ValidateUtil.isValid(pageIn.get("state")) && !"0".equals(pageIn.get("state")), "BULLETIN.STATE = :BULLETIN.STATE", pageIn.get("state"))
	...
	public PageOut getUserListpage(PageIn pageIn) {
		String sql = "SELECT USER.ID, USER.NAME AS NAME "
				+ "FROM SYS_USER USER ";
		SqlUtil sqlUtil = new SqlUtil(sql);
		sqlUtil.addWhere(ValidateUtil.isValid(pageIn.get("name")), "(USER.NAME LIKE :USER.NAME)", "%" + pageIn.get("name") + "%")
				.addWhere(ValidateUtil.isValid(pageIn.get("id")), "EXISTS (SELECT 1 FROM EXM_BULLETIN Z WHERE Z.ID = :Z.ID AND Z.READ_USER_IDS LIKE CONCAT('%,', USER.ID, ',%'))", pageIn.get("id"))
	...
	public PageOut getOrgListpage(PageIn pageIn) {
		String sql = "SELECT ORG.ID, ORG.NAME AS NAME "
				+ "FROM SYS_ORG ORG ";
		SqlUtil sqlUtil = new SqlUtil(sql);
		sqlUtil.addWhere(ValidateUtil.isValid(pageIn.get("name")), "(ORG.NAME LIKE :ORG.NAME)", "%" + pageIn.get("name") + "%")
				.addWhere(ValidateUtil.isValid(pageIn.get("id")), "EXISTS (SELECT 1 FROM EXM_BULLETIN Z WHERE Z.ID = :Z.ID AND Z.READ_ORG_IDS LIKE CONCAT('%,', ORG.ID, ',%'))", pageIn.get("id"))
