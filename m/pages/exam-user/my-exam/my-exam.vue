    ...
    <xmky-layout :tabs="tabbarStore.examUser">
        <view class="myexam">
            <view class="myexam-head">
                <uni-search-bar v-model="queryForm.examName" bgColor="#fff" radius="10" placeholder="请输入考试名称"
                    class="query" @confirm="
                        () => {
                            myExamQuery(false);
                            todoExamQuery(false);
            ...
            <view class="myexam-main">
                ...
                ]" :current="curTabIndex" :scrollable="false" lineHeight="4rpx" lineWidth="350rpx" lineColor="#0D9DF6"
                    :activeStyle="{
                        fontSize: '32rpx',
                        color: '#333333'
                    }" :inactiveStyle="{
                        fontSize: '30rpx',
                        color: '#8F939C'
                    }" @change="(item) => (curTabIndex = item.index)"></uv-tabs>
                <scroll-view scroll-y="true" class="myexam-main__scroll" :style="{ height: taskListHeight + 'px' }">
                    <xmky-card v-if="curTabIndex === 0" v-for="(myExam, index) in myExamListpage.list" :key="index"
                        :preTxt="(index + 1).toString().padStart(2, '0')" :name="myExam.examName" tag-name="考试">
                        <template #content>
                            <view>
                            ...
                            <view>
                                <text>用时：</text>
                                <text class="myexam-main__value">{{ diffMinute(myExam.answerStartTime,
                                    myExam.answerEndTime) }}分钟</text>
                                <text>分数：</text>
                                <text class="myexam-main__value">
                                    {{ myExam.totalScore == null ? '-' : myExam.totalScore }} / {{ myExam.examTotalScore
                                        == null ? '-' : myExam.examTotalScore }}
                                </text>
                                <text>排名：</text>
                                <text class="myexam-main__value">{{ myExam.no == null ? '-' : myExam.no }} / {{
                                    myExam.userNum == null ? '-' : myExam.userNum }}</text>
                            </view>
                        </template>
                        <template #opt>
                    ...
                    <uni-load-more v-if="curTabIndex === 0 && myExamListpage.list?.length"
                        :status="myExamListpage.status"
                        :contentText="{ contentdown: '点击查看更多', contentrefresh: '加载中', contentnomore: '没有更多数据了' }"
                        @clickLoadMore="myExamQuery(true)"></uni-load-more>
                    <xmky-empty v-if="curTabIndex === 0 && !myExamListpage.list?.length"></xmky-empty>

                    <xmky-card v-if="curTabIndex === 1" v-for="(todoExam, index) in todoExamListpage.list" :key="index"
                        :preTxt="(index + 1).toString().padStart(2, '0')" :name="todoExam.examName" tag-name="考试">
                        <template #content>
                            <view>
                            ...
                            <view>
                                <text>分数：</text>
                                <text class="myexam-main__value">{{ todoExam.examPassScore }} / {{
                                    todoExam.examTotalScore }}</text>
                                <text>限时：</text>
                                <text class="myexam-main__value">{{ todoExam.examLimitMinute === 0 ? '无' :
                                    todoExam.examLimitMinute + '分钟' }}</text>
                            </view>
                        </template>
                        <template #opt>
                            <view class="myexam-main__opt">
                                <view>
                                    <text>距离考试开始：</text>
                                    <xmky-count-down :expireTime="todoExam.examStartTime"></xmky-count-down>
                                </view>
                                <button type="primary" @click="toExam(todoExam)"
                                    class="myexam-main__exam-in">进入考试</button>
                            </view>
                        </template>
                    </xmky-card>
                    <uni-load-more v-if="curTabIndex === 1 && todoExamListpage.list?.length"
                        :status="todoExamListpage.status"
                        :contentText="{ contentdown: '点击查看更多', contentrefresh: '加载中', contentnomore: '没有更多数据了' }"
                        @clickLoadMore="todoExamQuery(true)"></uni-load-more>
...
import { myExamListpage as _myExamListpage } from '@/api/my-exam';
...
async function toExam(myExam: MyExam) {
    uni.navigateTo({ url: `/pages/exam-user/my-exam/my-read?examId=${myExam.examId}` });
}
</script>
