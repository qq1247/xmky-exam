/*==============================================================*/
/* DBMS name:      MySQL 5.0                                    */
/* Created on:     2024/10/11 22:10:46                          */
/*==============================================================*/
...
drop table if exists EXM_MY_WRONG_QUESTION;

drop table if exists EXM_MY_COMMENT;

drop table if exists EXM_MY_FAV_QUESTION;

drop table if exists EXM_MY_EXAM;

drop table if exists EXM_MY_EXAM_HIS;

drop table if exists EXM_MY_MARK;

drop table if exists EXM_MY_SXE;

drop table if exists EXM_MY_EXAM_QUESTION;

drop table if exists EXM_MY_EXAM_QUESTION_HIS;
...
drop table if exists EXM_QUESTION_BANK;
...
drop table if exists SYS_REGIST_USER;

drop table if exists SYS_VER;

drop table if exists EXM_MY_EXER_TRACK;

drop table if exists EXM_MY_EXER_TRACK_MONTHLY;

drop table if exists EXM_COURSE;

drop table if exists EXM_COURSE_MATERIAL;

drop table if exists EXM_COURSE_QUESTION;

drop table if exists EXM_MY_COURSE_MATERIAL;

drop table if exists EXM_MY_COURSE_QUESTION;
...
(
   ...
   LOGIN_TYPE           int comment '登录方式（1：账号登录；2：临时登录；）数据字典：LOGIN_TYPE',
   ...
   SXES                 varchar(32) comment '防作弊（1：试题乱序；2：选项乱序；3：禁止考试中切屏；4：禁止浏览器调试）',
   USER_IDS             text comment '用户IDS',
   ORG_IDS              text comment '机构IDS',
   MARK_USER_IDS        text comment '阅卷用户IDS',
   RETAKE_NUM           int comment '重考次数',
...
(
   ...
   CHAPTER_TXT          varchar(512) comment '章节描述',
   TYPE                 int comment '类型 （1：章节；2：试题）',
   QUESTION_BANK_ID     int comment '题库ID',
...
(
   ...
   QUESTION_BANK_IDS    varchar(64) comment '题库IDS',
   ORG_IDS              text comment '机构IDS',
   USER_IDS             text comment '用户IDS',
   STATE                int comment '状态（0：删除；1：发布；2：暂停）',
   COMMENT_STATE        int comment '评论状态（1：是；2：否）',
   CREATE_USER_ID       int comment '创建用户ID',
   UPDATE_USER_ID       int comment '修改用户ID',
   UPDATE_TIME          datetime comment '修改时间',
   primary key (ID)
...
alter table EXM_EXER comment '练习';
...
(
   ...
   NO                   int comment '排序',
   SCORE                decimal(5,2) comment '分数',
   SCORES               varchar(128) comment '子分数',
   MARK_OPTIONS         varchar(8) comment '阅卷选项（2：答案无顺序；3：不分大小写；)',
...
(
   ...
   ANSWER_START_TIME      datetime comment '答题开始时间',
   ANSWER_END_TIME        datetime comment '答题结束时间',
   MARK_START_TIME      datetime comment '阅卷开始时间',
   MARK_END_TIME        datetime comment '阅卷结束时间',
...
/* Table: EXM_MY_SXE                                          */
/*==============================================================*/
create table EXM_MY_SXE
(
   ID                   int not null auto_increment comment 'id',
   EXAM_ID              int comment '考试ID',
   USER_ID              int comment '考试用户ID',
   TYPE                 int comment '作弊类型（3：禁止考试中切屏；4：禁止浏览器调试）',
   CONTENT              varchar(128) comment '作弊内容',
   UPDATE_TIME          datetime comment '修改时间',
   primary key (ID)
);

alter table EXM_MY_SXE comment '我的作弊';

/*==============================================================*/
/* Table: EXM_MY_EXAM_QUESTION                                       */
/*==============================================================*/
create table EXM_MY_EXAM_QUESTION
(
   ID                   int not null auto_increment comment '主键',
   EXAM_ID              int comment '考试ID',
   QUESTION_ID          int comment '试题ID',
...
/*==============================================================*/
/* Table: EXM_MY_EXAM_QUESTION_HIS                                   */
/*==============================================================*/
create table EXM_MY_EXAM_QUESTION_HIS
(
   ID                   int not null auto_increment comment '主键',
   EXAM_ID              int comment '考试ID',
   QUESTION_ID          int comment '试题ID',
...
(
   ...
   IMG_FILE_IDS         varchar(32) comment '图片IDS',
   VIDEO_FILE_ID        int comment '视频ID',
   MARK_TYPE            int comment ' 阅卷类型（1：客观题；2：主观题）',
   MARK_OPTIONS         varchar(8) comment ' 阅卷选项（2：答案无顺序；3：不分大小写；)',
   ANALYSIS             text comment '解析',
   SCORE                decimal(5,2) comment '默认分值',
   STATE                int comment '状态（0：删除；1：发布；）',
   QUESTION_BANK_ID     int comment '试题分类ID',
   CREATE_USER_ID       int comment '创建用户ID',
   UPDATE_USER_ID       int comment '修改用户ID',
   UPDATE_TIME          datetime comment '修改时间',
   primary key (ID)
...
/*==============================================================*/
/* Table: EXM_QUESTION_BANK                                     */
/*==============================================================*/
create table EXM_QUESTION_BANK
(
   ID                   int not null auto_increment comment '主键',
   NAME                 varchar(32) comment '名称',
   OBJECTIVE_NUM        int comment '客观题数量',
   SUBJECTIVE_NUM       int comment '主观题数量',
   SINGLE_NUM           int comment '单选题数量',
   MULTIPLE_NUM         int comment '多选题数量',
   FILL_BLANK_OBJ_NUM   int comment '填空题客观数量',
   FILL_BLANK_SUB_NUM   int comment '填空题主观数量',
   JUDGE_NUM            int comment '判断题数量',
   QA_OBJ_NUM           int comment '问答题客观数量',
   QA_SUB_NUM           int comment '问答题主观数量',
   QUESTION_NUM         int comment '试题总数量',
   SHARE_AUTH           int comment '共享权限（1：私有；2：只读；3：读写；）',
   STATE                int comment '状态（0：删除；1：正常；）',
   FILE_IDS             varchar(64) comment '附件IDS',
   CREATE_USER_ID       int comment '创建用户ID',
   UPDATE_USER_ID       int comment '修改用户ID',
   UPDATE_TIME          datetime comment '修改时间',
   primary key (ID)
...
alter table EXM_QUESTION_BANK comment '题库';
...
(
   ...
   SYS_NAME             varchar(32) comment '系统名称',
   ...
   ICP                  varchar(512) comment '备案信息',
   M_HOST               varchar(128) comment '移动端地址',
   VERHUB_URL           varchar(128) comment '版本中心链接',
   APP_ID               varchar(32) comment '应用ID',
   APP_VER              varchar(32) comment '应用版本',
   APP_REL_VER          varchar(32) comment '应用发布版本',
   APP_REL_TIME         datetime comment '应用发布时间',
   USER_REGIST          int comment '用户注册（1：是；2：否）',
   UPDATE_USER_ID       int comment '修改用户ID',
   UPDATE_TIME          datetime comment '修改时间',
   primary key (ID)
);
...
/*==============================================================*/
/* Table: EXM_MY_EXER_TRACK_MONTHLY                             */
/*==============================================================*/
...
INSERT INTO `SYS_USER` VALUES (1, '管理员', 'admin', '79nRuL+wDo42R5kPfXTR2A==', 'ADMIN', NULL, NULL, NULL, NULL, NULL, NULL, '2017-08-01 22:31:43', '2017-08-01 22:31:43', 0, '管理员添加', 1, 1, '2017-08-01 22:31:43');
INSERT INTO `SYS_PARM` VALUES (1, NULL, NULL, NULL, NULL, NULL, '小猫考试', 'bak\\file', 'bak\\db', 2, '111111', '服务支持', '开源项目：小猫考试\r\n在线支持： qq群（811189776）', '小猫开源 - 在线考试&lt;br/&gt;\n&copy; 2017-2025 \n&lt;a href=&quot;https://beian.miit.gov.cn&quot; target=&quot;_blank&quot; rel=&quot;noopener noreferrer&quot;&gt;\n  晋ICP备2022004812号-4\n&lt;/a&gt;', NULL, 'http://verhub.xiaomaokaiyuan.com/api/ver/latest', REPLACE(UUID(), '-', ''), '6.1.0', '6.1.0', '2025-10-28 09:24:00', 2, 1, '2017-08-01 22:31:43');
...
INSERT INTO `SYS_DICT` VALUES (56, 'LOGIN_TYPE', '1', '账号登录', 1);
INSERT INTO `SYS_DICT` VALUES (57, 'LOGIN_TYPE', '2', '临时登录', 2);

INSERT INTO `SYS_DICT` VALUES (58, 'STATE_PS', '1', '发布', 1);
INSERT INTO `SYS_DICT` VALUES (59, 'STATE_PS', '2', '暂停', 2);
...
INSERT INTO `SYS_VER` VALUES (39, '4.6.1', '2025-01-25 18:06:00', 'zhanghc', '');
INSERT INTO `SYS_VER` VALUES (40, '5.0.0', '2025-02-20 14:56:00', 'zhanghc', '');
INSERT INTO `SYS_VER` VALUES (41, '5.1.0', '2025-03-27 17:12:00', 'zhanghc', '');
...
ALTER TABLE `EXM_MY_EXAM_QUESTION` ADD INDEX `MY_QUESTION_EUQ` ( `EXAM_ID`,`USER_ID`,`QUESTION_ID` );
