...
onMounted(() => {
    if (route.path.indexOf('cron/add') !== -1) {
        tabs.push({
            ...
            desc: '添加',
            icon: 'icon-edit',
            url: `/cron/add/${route.params.parentId}`,
        })
    } else {
        tabs.push({
            title: '定时任务信息',
            desc: '修改',
            icon: 'icon-edit',
            url: `/cron/edit/${route.params.id}`,
        })
        tabs.push({
            title: '删除',
            desc: '删除定时任务',
            icon: 'icon-delete',
            url: `/cron/del/${route.params.id}`,
        })
        tabs.push({
            title: '启动停止',
            desc: '启动停止',
            icon: 'icon-start',
            url: `/cron/start/${route.params.id}`,
        })
        tabs.push({
            title: '执行一次',
            desc: '执行一次',
            icon: 'icon-once',
            url: `/cron/runOnce/${route.params.id}`,
        })
    }
})
