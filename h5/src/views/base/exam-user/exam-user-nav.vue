<template>
    <div class="exam-user-nav">
        <xmks-sub-nav :nav-list="navList" @go="$router.push('/base-nav/exam-user-list')"></xmks-sub-nav>
    </div>
</template>
...
onMounted(async () => {
    if (route.path.indexOf('/add') !== -1) {
        navList.value.push(
            { 'title': '设置', 'url': `/exam-user-nav/add` }
        );
    } else {
        navList.value.push(
            { 'title': '设置', 'url': `/exam-user-nav/set/${route.params.id}` },
        );
    }
})
