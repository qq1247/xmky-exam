<template>
    <el-card class="edit" shadow="never">
        <template #header>
            ...
            <div class="edit-desc">{{ $route.path.indexOf('subAdmin/add') !== -1 ? '添加' : '修改' }}</div>
        </template>
        <el-form ref="formRef" :model="form" :rules="formRules" label-width="100" size="large">
            <el-form-item label="子管理名称" prop="name">
                <el-input v-model="form.name" placeholder="请输入子管理名称"/>
            </el-form-item>
...
const formRules = reactive<FormRules>({// 表单校验规则
    ...
    name: [
        { required: true, message: '请输入子管理名称', trigger: 'blur' },
