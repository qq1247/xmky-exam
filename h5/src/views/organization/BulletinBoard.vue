  ...
  <div class="container">
    ...
    <el-dialog :visible.sync="editForm.show" title="公告栏">
      <el-form :model="editForm" :rules="editForm.rules" ref="editForm">
        ...
        <el-form-item label="是否在首页展示" label-width="120px" prop="topState">
          <el-input
  ...
  methods: {
    ...
    add() {
      this.$refs["editForm"].validate(async (valid) => {
        ...
        let { code, msg } = await this.$https.bulletinBoardAdd({
    ...
    edit() {
      this.$refs["editForm"].validate(async (valid) => {
        ...
        let { code, msg } = await this.$https.bulletinBoardEdit({
          id: this.editForm.id,
          title: this.editForm.title,
          imgs: this.editForm.imgs,
          video: this.editForm.video,
    ...
    // 获取试题
    async get(id) {
      let res = await this.$https.bulletinBoardGet({ id: id });
      if (res.code != 200) {
        alert(res.msg);
        return;
      }
    ...
    async del(id) {
      this.$confirm("确定要删除？", "提示", {
        ...
        let res = await this.$https.bulletinBoardDel({ id });
        if (res.code != 200) {
          this.$message({
            type: "error",
            message: res.msg,
