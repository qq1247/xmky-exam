    ...
    <div class="content">
      ...
      <div class="table">
        <el-table :data="listpage.list" style="width: 100%">
          ...
          <el-table-column label="考试用户id">
            <template slot-scope="scope">
              <span style="margin-left: 10px">{{ scope.row.userId }}</span>
            </template>
          </el-table-column>
          <el-table-column label="考试用户名称">
            <template slot-scope="scope">
              <span style="margin-left: 10px">{{ scope.row.userName }}</span>
            </template>
          </el-table-column>
          <el-table-column label="答题开始时间">
            <template slot-scope="scope">
              <span style="margin-left: 10px">{{ scope.row.answerStartTimeStr }}</span>
            </template>
          </el-table-column>
          <el-table-column label="答题结束时间">
            <template slot-scope="scope">
              <span style="margin-left: 10px">{{ scope.row.answerEndTimeStr }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column label="阅卷状态">
            <template slot-scope="scope">
              <span style="margin-left: 10px">{{ scope.row.markState }}</span>
            </template>
          </el-table-column>
          <el-table-column label="阅卷状态名称">
            <template slot-scope="scope">
              <span style="margin-left: 10px">{{ scope.row.markStateName }}</span>
            </template>
          </el-table-column>
          <el-table-column label="答题状态">
            <template slot-scope="scope">
              <span style="margin-left: 10px">{{ scope.row.answerState }}</span>
            </template>
          </el-table-column>
          <el-table-column label="答题状态名称">
            <template slot-scope="scope">
              <span style="margin-left: 10px">{{ scope.row.answerStateName }}</span>
            </template>
          </el-table-column>
  ...
  methods: {
    ...
    add() {
      this.$refs["editForm"].validate(async (valid) => {
        ...
        let { code, msg } = await this.$https.myExamAdd({
          no: this.editForm.no,
        });
        if (code != 200) {
          alert(msg);
    ...
    edit() {
      this.$refs["editForm"].validate(async (valid) => {
        ...
        let { code, msg } = await this.$https.myExamEdit({
          id: this.editForm.id,
          no: this.editForm.no,
        });
        if (code != 200) {
    ...
    // 获取试题
    async get(id) {
      let res = await this.$https.myExamGet({ id: id });
      if (res.code != 200) {
        alert(res.msg);
        return;
      }
    ...
    async del(id) {
      this.$confirm("确定要删除？", "提示", {
        ...
        let res = await this.$https.myExamDel({ id });
        if (res.code != 200) {
          this.$message({
            type: "error",
            message: res.msg,
