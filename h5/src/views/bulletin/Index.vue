<template>
    <template v-if="$route.path === '/bulletin'">
        <el-form :inline="true" :model="queryForm" size="large" class="query">
            ...
            <el-form-item style="float: right;">
                <el-button type="success" @click="$router.push('/bulletin/add')">
                ...
                <template #default="scope">
                    <el-tooltip ffect="dark" content="修改">
                        <Iconfont icon="icon-edit" :size="18" color="#409eff"
                            @click="$router.push(`/bulletin/edit/${scope.row.id}`)"></Iconfont>
                    </el-tooltip>
                    <el-tooltip effect="dark" content="删除">
                        <Iconfont icon="icon-delete" :size="18" color="#409eff"
                            @click="$router.push(`/bulletin/del/${scope.row.id}`)"></Iconfont>
                    </el-tooltip>
                </template>
            </el-table-column> 
        </el-table>
...
async function query() {
    const { data: { code, data } } = await http.post('bulletin/listpage', {
