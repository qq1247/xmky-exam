<template>
    <div class="cron">
        <div class="cron__head">
            <div class="opt">
                <el-button type="success" class="opt__btn" @click="$router.push('/cron-nav/add')">
                    ...
                    <template #default="scope">
                        <el-tooltip ffect="dark" content="设置">
                            <span class="iconfont icon-liebiao-01 table__btn"
                                @click="$router.push(`/cron-nav/set/${scope.row.id}`)"></span>
                        </el-tooltip>
                    </template>
                </el-table-column>
            </el-table>
...
import { cronListpage } from '@/api/sys/cron'
...
async function query() {
    const { data: { code, data } } = await cronListpage({
        ...queryForm,
